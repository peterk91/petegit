...
<?php	session_start();
	...
	$stateSQL = 'SELECT * FROM Request WHERE UserID = "'.$_SESSION['systemlogged1'].'" AND RequestID NOT IN(SELECT RequestID FROM Denied) AND RequestID NOT IN(SELECT RequestID FROM Allocation) '.$sem.' '.$round.' ORDER BY Date DESC';
	...
	while($row=mysql_fetch_array($result2)){
		...
		echo 'pendingrecord['.$count.']=record ; ';
		$count++;};
