//This script makes an array of records of all the allocated requests for the user.
...
<?php session_start();
	...
	$stateSQL = 'SELECT * FROM Request WHERE UserID = "'.$_SESSION['systemlogged1'].'" AND RequestID IN(SELECT RequestID FROM Allocation) '.$sem.' '.$round.' ORDER BY Date DESC';	$result2=mysql_query($stateSQL, $myconn2);
	$count=0;
	while($row=mysql_fetch_array($result2)){
		...
		$stateSQL2 = 'SELECT * FROM RequestRelease WHERE RequestID = "'.$row["RequestID"].'"';
		$result3=mysql_query($stateSQL2, $myconn2);
		$found = 'no';
		while($row2=mysql_fetch_array($result3)){$found='yes';}
		echo 'record.release= "'.$found.'" ; ';
