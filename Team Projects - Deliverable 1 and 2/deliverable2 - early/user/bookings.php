...
function makebuildlist(chosen,x){
	...
	for(y=0;y<buildnameslist[x].length;y++){
		select1 += '<option value ="'+buildcodeslist[x][y]+'">'+buildcodeslist[x][y]+'</option>'
		select2 += '<option value ="'+buildnameslist[x][y]+'">'+buildnameslist[x][y]+'</option>'}
	select1 += '</select>'
	select2 += '</select>'
...
function passlastReqID(lastReqID){
	...
	for(i=0;i<strArray.length;i++) alert(strArray[i]);
